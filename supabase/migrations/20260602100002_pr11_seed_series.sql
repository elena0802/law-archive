-- PR11: Seed curated series volumes (from archiveSeriesTitles + lib/series.ts)
-- Essay import is a separate step; this prepares FK targets for essays.series_slug.

INSERT INTO series (slug, title, description, display_order)
VALUES
  (
    '형벌과-사회',
    '형벌과 사회',
    '국가 형벌권과 공동체의 응답을 형사법의 관점에서 차분히 정리하는 연재입니다. 복수의 감정과 제도적 형벌이 만나는 지점에서, 우리는 무엇을 묻어야 하는가.',
    0
  ),
  (
    'ai와-형사법',
    'AI와 형사법',
    '디지털 기술이 수사·재판·책임 개념에 던지는 질문을 형사법의 언어로 따라갑니다. 도구의 시대에도 남는 판단과 윤리의 자리를 묻습니다.',
    1
  ),
  (
    '로스쿨-시대',
    '로스쿨 시대',
    '법학 교육과 법조인의 사유 방식을 돌아보는 연재입니다. 강의실에서 시작된 질문이 어떻게 오래 가는 사유가 되는지 기록합니다.',
    2
  ),
  (
    '형사법-교수로-산다는-것',
    '형사법 교수로 산다는 것',
    '형사법을 가르치고 연구하며 살아온 시간의 기록입니다. 학자로서의 일상과 강의실의 질문을 차분히 남깁니다.',
    3
  ),
  (
    '법과-인간',
    '법과 인간',
    '법이 인간을 어디까지 바꿀 수 있는지, 그리고 어디까지 바꿔서는 안 되는지를 묻는 연재입니다. 제도 너머의 인간을 향한 사유입니다.',
    4
  ),
  (
    '형사소송의-질문들',
    '형사소송의 질문들',
    '수사와 공판, 증거와 방어권을 관통하는 형사소송법의 기본 질문을 차분히 따라갑니다. 절차는 형벌의 문 앞에 놓인 또 하나의 법입니다.',
    5
  ),
  (
    '판례를-읽는-방법',
    '판례를 읽는 방법',
    '판례 문장을 어떻게 읽고, 무엇을 물어야 하는지를 기록하는 연재입니다. 결론보다 이유와 구조를 오래 붙들기 위한 독법입니다.',
    6
  ),
  (
    '법학자의-서재',
    '법학자의 서재',
    '책·원고·강의 노트가 쌓이는 학자의 서재를 통해, 형사법을 생각하는 방식을 기록합니다. 이 디지털 서재가 놓인 자리를 설명합니다.',
    7
  )
ON CONFLICT (slug) DO UPDATE SET
  title = EXCLUDED.title,
  description = EXCLUDED.description,
  display_order = EXCLUDED.display_order;
